...
def submitReview():
    ...
    if (form_data.getvalue('submit')):
      ...
      reviewData.append(form_data.getfirst('author'))

      if (reviewComplete(reviewData)):
        insertReview(connect(dsn),reviewData)
      else:
        print main() # TODO: print out form with current user input
...
def insertReview(conn,reviewData):
  ...
  data= (reviewData[1],reviewData[5],reviewData[0],reviewData[3],reviewData[2],)
  curs.execute('insert into review(pid,uid,name,title,reviewText,rating) values (1,%s,%s,%s,%s,%s)',data)
  print main()
...
def main():
    ...
    tmpl =  file_contents('bootstrapreview.html')
    return tmpl
